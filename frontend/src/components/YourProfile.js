...
export default function YourProfile() {
  ...
  const fetchUserInfo = async (userId) => {
    try {
      const response = await axios.get(`https://hostel-hunt-1.onrender.com/api/profile/${userId}`);
  ...
  const fetchUserItems = async (userId) => {
    try {
      const response = await axios.get(`https://hostel-hunt-1.onrender.com/api/user-items/${userId}`);
  ...
  const handleMarkAsSold = async (itemId) => {
    try {
      ...
      await axios.delete(`https://hostel-hunt-1.onrender.com/api/items/${itemId}`, { data: { userId } });
